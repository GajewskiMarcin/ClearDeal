...
class AdminWiseBlockBlockController extends ModuleAdminController
{
    ...
    public function renderForm()
    {
        ...
        if ($isEdit) {
            foreach ($languages as $lang) {
                $langData = Db::getInstance()->getRow('SELECT title, content FROM '._DB_PREFIX_.'wiseblock_block_lang WHERE id_block='.(int)$obj->id.' AND id_lang='.(int)$lang['id_lang']);
        ...
        if ($hasMultipleLanguages) {
            $html .= '<div class="wb-form-label-row" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                <label class="wb-form-label" style="margin-bottom:0;">'.$this->_l('HTML Content').'</label>
                <div class="wb-lang-selector" id="wb-lang-selector" style="position:relative;display:inline-block;">
                    <button type="button" class="wb-lang-selector-btn" onclick="toggleLangDropdown(event)" style="display:inline-flex;align-items:center;gap:8px;padding:6px 12px;background:white;border:1px solid #e5e7eb;border-radius:8px;font-size:13px;font-weight:500;color:#374151;cursor:pointer;">
                        <img src="../img/l/'.$defaultLang.'.jpg" alt="" class="wb-lang-flag" id="wb-current-lang-flag" style="width:20px;height:14px;border-radius:2px;">
                        <span id="wb-current-lang-code">'.strtoupper($languages[array_search($defaultLang, array_column($languages, 'id_lang'))]['iso_code'] ?? 'EN').'</span>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <div class="wb-lang-dropdown" id="wb-lang-dropdown" style="position:absolute;top:calc(100% + 4px);right:0;min-width:180px;background:white;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 10px 40px rgba(0,0,0,0.12);z-index:1000;display:none;">';

            foreach ($languages as $lang) {
                $isActive = $lang['id_lang'] == $defaultLang;
                $html .= '<button type="button" class="wb-lang-option '.($isActive ? 'active' : '').'" onclick="switchLanguage('.$lang['id_lang'].', \''.strtoupper($lang['iso_code']).'\', this)" style="display:flex;align-items:center;gap:10px;width:100%;padding:10px 14px;background:'.($isActive ? 'rgba(99,102,241,0.08)' : 'white').';border:none;font-size:14px;color:'.($isActive ? '#6366f1' : '#374151').';cursor:pointer;text-align:left;">
            ...
            $html .= '<label class="wb-form-label">'.$this->_l('HTML Content').'</label>';
        }

        // Textareas for each language (hidden, switched via JS)
        $textareaStyle = 'background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;font-family:Courier New,monospace;font-size:13px;line-height:1.6;width:100%;min-height:180px;box-sizing:border-box;resize:vertical;';
        foreach ($languages as $lang) {
            $isActive = $lang['id_lang'] == $defaultLang;
            $activeClass = $isActive ? ' active' : '';
            $html .= '<textarea name="content_'.$lang['id_lang'].'" class="wb-form-textarea wb-translatable-field'.$activeClass.'" data-lang="'.$lang['id_lang'].'" rows="10" placeholder="'.$this->_l('Enter HTML content for').' '.htmlspecialchars($lang['name']).'..." style="'.$textareaStyle.'">'.htmlspecialchars($contents[$lang['id_lang']] ?? '').'</textarea>';
        }

        $html .= '<div class="wb-placeholders-section">
            ...
            <div class="wb-placeholders-content" style="display:none;">
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Shop').'</div>
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Free Shipping').'</div>
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Stock').'</div>
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Price').'</div>
                    <code onclick="insertPlaceholder(\'{{#if_on_sale}}...{{/if_on_sale}}\')">{{#if_on_sale}}</code>
                </div>
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Cart').'</div>
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Customer').'</div>
                ...
                <div class="wb-placeholders-group">
                    <div class="wb-placeholders-group-title">'.$this->_l('Conditional Blocks').' - '.$this->_l('Product').'</div>
    ...
    protected function renderFormJavaScript()
    {
        ...
        function selectLogic(value, card) {
            document.querySelectorAll(".wb-logic-card").forEach(function(c) {
        ...
        function toggleLangDropdown(event) {
            ...
            var dropdown = document.getElementById("wb-lang-dropdown");
            if (!dropdown) return;
        ...
        function switchLanguage(langId, langCode, btn) {
            ...
            var langCodeEl = document.getElementById("wb-current-lang-code");
            var langFlagEl = document.getElementById("wb-current-lang-flag");
            if (langCodeEl) langCodeEl.textContent = langCode;
            if (langFlagEl) langFlagEl.src = "../img/l/" + langId + ".jpg";

            // Update active states in dropdown
            document.querySelectorAll(".wb-lang-option").forEach(function(opt) {
            ...
            // Switch visible textarea using classes
            document.querySelectorAll(".wb-translatable-field").forEach(function(field) {
                field.classList.remove("active");
            });
            var targetField = document.querySelector(".wb-translatable-field[data-lang=\'" + langId + "\']");
            if (targetField) {
        ...
        document.addEventListener("click", function(e) {
            var selector = document.getElementById("wb-lang-selector");
            var dropdown = document.getElementById("wb-lang-dropdown");
        ...
        function insertPlaceholder(placeholder) {
            var activeTextarea = document.querySelector(".wb-translatable-field.active");
            if (!activeTextarea) {
                activeTextarea = document.querySelector(".wb-translatable-field");
            }
            if (activeTextarea) {
