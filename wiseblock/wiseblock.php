...
class WiseBlock extends Module
{
    ...
    /** Render blocks for given hook on product page */
    private function renderForHook($hookName, $params)
    {
        ...
        // get product categories (with tree) and tags
        ...
        foreach ($blocks as $b) {
            ...
            if ($this->matchesRules((int)$b['id_block'], $categoryIds, $tagIds, $id_lang, $b['logic_mode'], $id_product)) {
                $content = $this->renderPlaceholders($b['content'], $id_product, $id_lang, $id_shop);
    ...
    /** Check if block should be shown based on time_from and time_to */
    ...
    /** Placeholder renderer with whitelist */
    private function renderPlaceholders($html, $id_product, $id_lang, $id_shop)
    {
        ...
        // Product-based placeholders
        $replacements = array(
            '{{product_name}}' => pSQL($product->name, true),
            '{{reference}}' => pSQL($product->reference, true),
            '{{ean13}}' => pSQL($product->ean13, true),
            '{{manufacturer}}' => pSQL(Manufacturer::getNameById((int)$product->id_manufacturer), true),
            '{{category_name}}' => pSQL($this->getDefaultCategoryName($product, $id_lang), true),
            '{{category_description}}' => $this->getDefaultCategoryDescription($product, $id_lang),
            ...
            '{{price}}' => $locale->formatPrice($product->getPrice(true), $currency),
            '{{price_without_tax}}' => $locale->formatPrice($product->getPrice(false), $currency),
            '{{stock_status}}' => $product->quantity > 0 ? $this->l('In Stock') : $this->l('Out of Stock'),
            '{{stock_quantity}}' => (int)$product->quantity,
            '{{weight}}' => $product->weight > 0 ? number_format($product->weight, 2) . ' ' . Configuration::get('PS_WEIGHT_UNIT') : '',
        );
        ...
        // {{#if_new_product}}...{{/if_new_product}} - based on PS "new product" days setting
        $isNew = $product->isNew();
        if (!$isNew) {
    ...
    /** Hook displayHeader - inject custom CSS and save UTM params */
